...
const TEACHER_WORKSPACE_PREFERENCES_EVENT = "teacher-workspace-preferences:change";
...
export function useTeacherWorkspacePreferences(
  ...
  useEffect(() => {
    ...
    window.addEventListener(TEACHER_WORKSPACE_PREFERENCES_EVENT, onPreferencesChanged);

    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener(TEACHER_WORKSPACE_PREFERENCES_EVENT, onPreferencesChanged);
    };
  }, [storageKey]);
