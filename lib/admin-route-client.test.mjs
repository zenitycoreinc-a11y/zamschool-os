...
import {
  ...
  parseAdminRouteResponse,
  resetAdminRouteAuthTokenProvider,
  setAdminRouteAuthTokenProvider,
} from "./admin-route-client.ts";

test("parseAdminRouteResponse throws the route error message", async () => {
  const response = new Response(JSON.stringify({ error: "Failed to fetch finance records" }), {
    status: 500,
    headers: { "Content-Type": "application/json" },
  });

  await assert.rejects(() => parseAdminRouteResponse(response), /Failed to fetch finance records/);
});
...
test("adminGet sends a GET request with credentials included", async () => {
  ...
  setAdminRouteAuthTokenProvider(() => "token-123");
