...
export async function loadTeacherStudentIntelligence(input: {
  ...
  const [attendanceRows, resultRows] = await Promise.all([
    loadAttendanceRows(input.schoolId, assignmentScope.allowedClassIds, Array.from(studentIdentityKeys)),
  ...
  for (const row of attendanceRows) {
    const studentKey = studentKeyByIdentity.get(String(row.student_id || "").trim());
  ...
  const studentsWithSignals = students.map((student) => {
    ...
    const attendanceRate =
      attendance.total > 0
        ? Math.round(
            ((attendance.present + attendance.late + attendance.excused) / attendance.total) * 100
          )
        : null;
    const lowScoreCount = allScoredPercentages.filter((value) => value < 50).length;
  ...
  return {
    ...
    summary: {
      ...
      highRiskStudents: studentsWithSignals.filter((student) => student.riskLevel === "high").length,
      mediumRiskStudents: studentsWithSignals.filter((student) => student.riskLevel === "medium").length,
    },
  };
}
...
async function loadStudentsByClassIds(schoolId: string, classIds: string[]) {
  ...
  if (!studentError) {
    ...
    return (studentRows || [])
      ...
      .map((row: any) => {
        const profile = profileMap.get(String(row.profile_id || "").trim()) || null;
        return {
          id: String(row.id),
  ...
  const { data: profileRows, error: profileError } = await supabaseAdmin
    .from("profiles")
    .select("id, class_id, first_name, last_name, email, role, admission_number")
    .eq("school_id", schoolId)
...
async function loadProfilesByIds(profileIds: string[]) {
  if (profileIds.length === 0) {
    return new Map<string, { id: string; email: string | null } & Record<string, unknown>>();
  }
...
async function loadAttendanceRows(schoolId: string, classIds: string[], studentIdentityKeys: string[]) {
...
async function loadResultRows(schoolId: string, studentIdentityKeys: string[]) {
  ...
  const modern = await supabaseAdmin
    .from("results")
    .select("id, student_id, assessment_type, score, max_score, date, remarks, created_at")
    .eq("school_id", schoolId)
    .in("student_id", studentIdentityKeys)
    .order("date", { ascending: false });

  if (!modern.error) {
    return (modern.data || []) as RawResultRow[];
  }

  const fallback = await supabaseAdmin
    .from("results")
    .select("id, student_id, score, max_score, date, created_at")
    .eq("school_id", schoolId)
    .in("student_id", studentIdentityKeys)
    .order("date", { ascending: false });
  ...
  if (!fallback.error) {
    return (fallback.data || []).map((row: any) => ({
      ...row,
      assessment_type: row.assessment_type || "Assessment",
      remarks: row.remarks || null,
    })) as RawResultRow[];
  }

  throw fallback.error || modern.error;
}
...
async function loadClassesById(schoolId: string, classIds: string[]) {
  ...
  if (!legacy.error) {
    return new Map((legacy.data || []).map((row: any) => [String(row.id), buildClassLabel(row)]));
  }
  ...
  return new Map((modern.data || []).map((row: any) => [String(row.id), buildClassLabel(row)]));
}
...
function deriveRiskLevel(flags: string[]): TeacherStudentRiskLevel {
  if (flags.includes("Low attendance") || flags.includes("Low performance") || flags.includes("Repeated low scores")) {
    return "high";
  }

  if (flags.includes("Attendance watch") || flags.includes("Performance watch") || flags.includes("Frequent lateness")) {
    return "medium";
  }
...
function buildClassHealth(students: TeacherStudentIntelligenceRow[]) {
  ...
  return Array.from(grouped.entries()).map(([classId, rows]) => {
    ...
    return {
      ...
      highRiskCount: rows.filter((student) => student.riskLevel === "high").length,
      mediumRiskCount: rows.filter((student) => student.riskLevel === "medium").length,
      averageAttendanceRate:
        attendanceRates.length > 0
          ? Math.round(attendanceRates.reduce((sum, value) => sum + value, 0) / attendanceRates.length)
          : null,
      averageScore:
        averageScores.length > 0
          ? Math.round(averageScores.reduce((sum, value) => sum + value, 0) / averageScores.length)
          : null,
