  ...
  useEffect(() => {
    ...
    const loadAttendance = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(payload?.error || "Failed to load attendance summary");
        }
        if (cancelled) return;
      ...
      <div className="mt-2 h-[280px] min-h-[220px] w-full min-w-0">
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height="100%" minWidth={0}>
            <BarChart width={500} height={300} data={data} barSize={18} margin={{ top: 12, right: 0, left: -20, bottom: 0 }}>
