...
import { loadDashboardScope } from "@/lib/dashboard-client";
  ...
  useEffect(() => {
    ...
    const loadFinance = async () => {
      try {
        ...
        setRows((data || []).filter((row: any) => !row.school_id || row.school_id === scope.schoolId));
      } catch (error) {
...
async function resolveTable() {
  const candidates = ["payments", "fee_payments", "school_payments"];
