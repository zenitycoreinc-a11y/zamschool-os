...
test("admin dashboard widgets read live data instead of baked-in demo placeholders", async () => {
  ...
  assert.match(countChart, /supabase/);
  assert.match(countChart, /Unknown|unspecified/i);
  ...
  assert.match(financeChart, /from\("payments"\)|from\("finance_entries"\)|resolveTable/);
