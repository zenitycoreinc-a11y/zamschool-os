...
export async function GET(req: Request) {
  try {
    ...
    const [profileExtras, legacyStudent] = await Promise.all([
      loadProfileExtras(userId, schoolId),
      loadLegacyStudentRecord(userId, schoolId),
    ]);

    const classId = profileExtras.classId || legacyStudent?.classId || null;
    const admissionNumber =
      profileExtras.admissionNumber || legacyStudent?.studentNumber || null;
    const { classRow, gradeLabel } = await loadClassContext(classId, profileExtras.gradeId);
    ...
    const [lessonsResult, attendanceState, assignmentsResult] = await Promise.all([
      supabaseAdmin
        .from("lessons")
    ...
    let attendanceLessons: any[] = [];
    if (attendanceLessonIds.length > 0) {
...
async function loadProfileExtras(userId: string, schoolId: string) {
  const { data, error } = await supabaseAdmin
    .from("profiles")
    .select("admission_number, class_id, grade_id")
    .eq("id", userId)
    .eq("school_id", schoolId)
    .maybeSingle();
