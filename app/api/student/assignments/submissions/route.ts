...
const submissionSchema = z
  ...
  .refine(
    (value) => Boolean(String(value.submissionText || "").trim() || String(value.submissionLink || "").trim()),
    {
      message: "Add submission text or a submission link before saving.",
      path: ["submissionText"],
    }
  );
...
export async function POST(req: Request) {
  try {
    ...
    const { data: savedSubmission, error: saveError } = await supabaseAdmin
      ...
      .select("id, assignment_id, student_profile_id, submission_text, submission_link, submitted_at, updated_at")
      .single();
