...
export async function POST(req: Request) {
  try {
    ...
    if (!shouldRequireFirstLoginPasswordChange(role)) {
      return NextResponse.json(
        { error: "Temporary password reset is only available for managed accounts." },
        { status: 400 }
      );
    }
    ...
    if (!authUser) {
      ...
      if (!email) {
        return NextResponse.json({ error: "Managed account email is missing." }, { status: 400 });
      }
      ...
      if (createdAuth.error || !createdAuth.data.user) {
        throw createdAuth.error || new Error("Failed to repair managed auth user");
      }
    ...
    const userMetadata = {
      ...
      first_name: (authUser.user_metadata?.first_name as string | undefined) || String(profile.first_name || "").trim() || "User",
      last_name: (authUser.user_metadata?.last_name as string | undefined) || String(profile.last_name || "").trim(),
    };

    if (authLookup.data.user) {
      const updateAuth = await supabaseAdmin.auth.admin.updateUserById(profile.id, {
        password: temporaryPassword,
