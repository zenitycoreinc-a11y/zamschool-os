...
const pagePath = resolve(process.cwd(), "app", "app", "admin", "users", "page.tsx");
const routePath = resolve(process.cwd(), "app", "api", "admin", "users", "route.ts");
const teacherDetailPath = resolve(process.cwd(), "lib", "teacher-account-detail.ts");
...
test("admin user route sets first-login flags for managed roles", () => {
  ...
  for (const role of ["student", "teacher", "parent"]) {
    ...
    assert.equal(payload.must_change_password, true, `${role} should require a first-login password change`);
    assert.equal(
...
test("admin accounts are not forced through first-login reset", () => {
  const now = new Date("2026-03-19T12:34:56.000Z");
  ...
  assert.equal(payload.temporary_password_issued_at, null);
});
...
test("auth metadata carries the first-login fallback flag", () => {
  ...
  assert.equal(adminMetadata.must_change_password, false);
});
...
test("admin users route fails open if Redis-backed rate limiting is unavailable", async () => {
  const source = await readFile(routePath, "utf8");

  assert.match(source, /key:\s*`admin-users:\$\{ip\}`/);
  assert.match(source, /key:\s*`admin-users-update:\$\{ip\}`/);
...
test("admin users route supports admin people drill-down details", async () => {
  ...
  assert.match(routeSource, /resolveExistingTable\(\["parent_students", "parent_student_links"\]\)/);
});
...
test("admin users route supports managed account edits through PUT", async () => {
  ...
  assert.match(source, /safeUpdateScoped\("profiles"/);
...
test("reset password route repairs managed accounts that are missing auth users", async () => {
  ...
  assert.match(source, /id:\s*profile\.id/);
});
