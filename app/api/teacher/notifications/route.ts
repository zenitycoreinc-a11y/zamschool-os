...
export async function GET(req: Request) {
  try {
    ...
    const rows = await loadNotifications(access.context.userId, limit);
...
export async function PUT(req: Request) {
  try {
    ...
    if (body?.markAll) {
      const updatedCount = await markAllNotificationsRead(access.context.userId);
    ...
    const updated = await markNotificationRead(id, access.context.userId);
    if (!updated) {
...
async function markAllNotificationsRead(userId: string) {
  ...
  for (const runMutation of mutationAttempts) {
    const result = await runMutation();
    if (!result.error) return Array.isArray(result.data) ? result.data.length : 0;
  }
