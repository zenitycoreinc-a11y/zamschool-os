...
export async function GET(req: Request) {
  try {
    ...
    const [studentRows, attendanceRows, classesById] = await Promise.all([
      getStudentsByClassIds({ schoolId, classIds }),
      getAttendanceRows({ schoolId, classIds, date }),
...
async function getStudentsByClassIds(input: {
  schoolId: string;
  classIds: string[];
}) {
  if (input.classIds.length === 0) {
...
async function getAttendanceRows(input: {
  schoolId: string;
  classIds: string[];
  date: string;
}) {
