...
import { Filter, SlidersHorizontal, Plus, Eye, Trash2, GraduationCap, X, Upload, Pencil } from "lucide-react";
...
export default function StudentList() {
  ...
  const visibleStudents = students.filter((item) => {
    ...
    if (!q) return true;
    return `${item.name} ${item.email || ""} ${item.admission_number || ""}`.toLowerCase().includes(q);
  });
    ...
    >
      <td className="flex items-center gap-4 p-4">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-[10px] text-slate-400 font-medium">{item.class || "N/A"}</p>
        </div>
      </td>
      <td className="hidden md:table-cell text-slate-500 font-medium">{item.admission_number || "STU-" + item.id.slice(0,4)}</td>
      <td className="hidden md:table-cell text-slate-500">{item.grade || "N/A"}</td>
      ...
      <td>
        <div className="flex items-center gap-2">
          ...
          <button 
            onClick={() => router.push(`/app/admin/users?tab=students&edit=${item.id}`)}
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex-1">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
        <h1 className="text-2xl font-bold text-slate-800 self-start md:self-center">Students</h1>
      ...
      <AnimatePresence>
        {showImport && (
          ...
          >
            <BulkImport role="STUDENT" onComplete={() => {
              setShowImport(false);
              fetchStudents();
            }} />
          </motion.div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Your student roster is currently empty. Add your first student to begin tracking their progress."
          actionLabel="Add Student"
          onAction={() => router.push("/app/admin/users")}
        />
      ) : (
        <div className="overflow-x-auto">
          <Table columns={columns} renderRow={renderRow} data={visibleStudents} />
        </div>
      )}
