...
export default function StudentDashboard() {
  ...
  async function saveAssignmentSubmission() {
    ...
    if (!submissionText && !submissionLink) {
      setSubmissionError("Add submission text or a submission link before saving.");
      return;
    }
        ...
        <div className="fixed inset-0 z-40 grid place-items-center bg-slate-900/40 p-4">
          <section className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              >
                {savingSubmission ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                {selectedAssignment.submissionStatus === "submitted" ? "Update submission" : "Submit work"}
              </button>
            </div>
          </section>
        </div>
