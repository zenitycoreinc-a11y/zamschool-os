...
const dashboardLayoutPath = resolve(process.cwd(), "app", "(dashboard)", "layout.tsx");
const workspaceLayoutPath = resolve(process.cwd(), "app", "app", "layout.tsx");
const landingPagePath = resolve(process.cwd(), "app", "page.tsx");

test("root layout stays public while protected layouts own auth gating", async () => {
  const [rootLayout, dashboardLayout, workspaceLayout] = await Promise.all([
  ...
  assert.match(dashboardLayout, /import AuthProvider from ['"]@\/components\/AuthProvider['"]/);
  assert.match(dashboardLayout, /<AuthProvider>/);
