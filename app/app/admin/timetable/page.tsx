...
export default function AdminTimetablePage() {
  ...
  const board = useMemo(
    () => buildTimetableBoard({ lessons, selectedClass, classMap, subjectMap, teacherMap }),
    [classMap, lessons, selectedClass, subjectMap, teacherMap]
  );
  const mobileSections = useMemo(
    () => buildMobileDaySections({ lessons, selectedClass, classMap, subjectMap, teacherMap }),
    [classMap, lessons, selectedClass, subjectMap, teacherMap]
  );
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const { data: me, error } = await supabase.from("profiles").select("school_id").eq("id", user.id).maybeSingle();
        if (error) throw error;
        if (!me?.school_id) throw new Error("No school linked to this account");
        setSchoolId(me.school_id);
        await fetchAll(me.school_id);
      } catch (err: any) {
  ...
  async function fetchAll(sid: string) {
    const [classesRes, subjectRes, teacherRes, lessonsRes] = await Promise.all([
      ...
      supabase.from("profiles").select("id,first_name,last_name,email").eq("school_id", sid).in("role", ["teacher", "TEACHER"]),
    ...
    const nextSubjects = (subjectRes.data || []) as SelectOpt[];
    const nextTeachers = (teacherRes.data || []).map((x: any) => ({ id: x.id, name: getDisplayName(x) }));
          ...
          <div key={section.key} className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="space-y-3">
              {section.lessons.map((lesson) => (
                <LessonCard key={lesson.id} lesson={lesson} compact menuOpen={menuLessonId === lesson.id} onToggleMenu={() => setMenuLessonId((current) => current === lesson.id ? null : lesson.id)} onView={() => { setDetailLessonId(lesson.id); setMenuLessonId(null); }} onDelete={() => void deleteLesson(lesson.id)} />
              ))}
            </div>
          </div>
        ))}
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[1080px] grid" style={{ gridTemplateColumns: "112px repeat(5, minmax(0, 1fr))" }}>
            ...
            {board.days[0]?.slots.map((slot, index) => (
              <ScheduleRow key={slot.label} slotLabel={slot.label} slotIndex={index} days={board.days} menuLessonId={menuLessonId} onToggleMenu={(id) => setMenuLessonId((current) => current === id ? null : id)} onView={(id) => { setDetailLessonId(id); setMenuLessonId(null); }} onDelete={(id) => void deleteLesson(id)} />
            ))}
          </div>
        </div>
      </section>
      ...
      {detailLesson && detailCard ? (
        <Modal title={detailCard.title} subtitle={`${DAY_OPTIONS.find((day) => Number(day.value) === detailLesson.day_of_week)?.label || "Day"} · ${detailCard.timeRange}`} onClose={() => setDetailLessonId(null)}>
          ...
          <div className="mt-6 flex justify-end gap-2">
            ...
            <button type="button" onClick={() => void deleteLesson(detailLesson.id)} className="inline-flex items-center gap-2 rounded-2xl bg-rose-600 px-4 py-3 text-sm font-semibold text-white"><Trash2 className="h-4 w-4" /> Delete lesson</button>
          </div>
        </Modal>
      ) : null}
    </div>
...
function ScheduleRow({ slotLabel, slotIndex, days, menuLessonId, onToggleMenu, onView, onDelete }: { slotLabel: string; slotIndex: number; days: ReturnType<typeof buildTimetableBoard>["days"]; menuLessonId: string | null; onToggleMenu: (id: string) => void; onView: (id: string) => void; onDelete: (id: string) => void; }) {
  return <>
    <Cell className="bg-slate-50 text-xs font-medium text-slate-500">{slotLabel}</Cell>
    {days.map((day) => {
      const slot = day.slots[slotIndex];
      return <Cell key={`${day.key}-${slot.label}`} className="align-top bg-white">{slot.lessons.length === 0 ? <div className="h-full rounded-2xl border border-dashed border-slate-100 bg-slate-50/60" /> : <div className="space-y-2">{slot.lessons.map((lesson) => <LessonCard key={lesson.id} lesson={lesson} menuOpen={menuLessonId === lesson.id} onToggleMenu={() => onToggleMenu(lesson.id)} onView={() => onView(lesson.id)} onDelete={() => onDelete(lesson.id)} />)}</div>}</Cell>;
    })}
  </>;
}

function LessonCard({ lesson, compact = false, menuOpen, onToggleMenu, onView, onDelete }: { lesson: LessonCardView; compact?: boolean; menuOpen: boolean; onToggleMenu: () => void; onView: () => void; onDelete: () => void; }) {
      ...
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <ActionMenu open={menuOpen} onToggle={onToggleMenu} onView={onView} onDelete={onDelete} />
          </div>
...
function ActionMenu({ open, onToggle, onView, onDelete }: { open: boolean; onToggle: () => void; onView: () => void; onDelete: () => void; }) {
  return <div className="relative"><button type="button" onClick={onToggle} className="grid h-9 w-9 place-items-center rounded-xl bg-white/80 text-slate-600 ring-1 ring-slate-200" aria-haspopup="menu" aria-expanded={open}><MoreHorizontal className="h-4 w-4" /></button>{open ? <div className="absolute right-0 z-10 mt-2 w-40 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-xl">{getLessonActionItems({ id: "x" }).map((action) => action.key === "view" ? <button key={action.key} type="button" onClick={onView} className="flex w-full rounded-xl px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50">{action.label}</button> : <button key={action.key} type="button" onClick={onDelete} className="flex w-full rounded-xl px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-50">{action.label}</button>)}</div> : null}</div>;
}
